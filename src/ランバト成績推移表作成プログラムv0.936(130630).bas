...
#define	ranbatokun	"<a href=\"http:\/\/www.yurica.net\/dam\/\">らんばと君 v2.12<\/a>(2012/10/19)<br>"
#define	ranbatohan	"<a href=\"http:\/\/hp.ckoshien.client.jp\/contents\/ranbat.html\">らんばとはん v"+ver+"<\/a><br>"
...
;v0.934以降高速化でnote系命令廃止
	...
	add_string "<script src=\"http://ajax.googleapis.com/ajax/libs/jquery/1.7.0/jquery.min.js\"></script>\n"
	add_string "<script src=\"jquery.tablefix_1.0.1.js\"></script>\n"
	add_string "<script>\n"
	add_string 	"$(function() {\n"
	...
	if sr(7)==0:add_string "<th class=\"right\">平均点</th>\n"
	...
	for cha,0,id,1;各曲出力ループ開始
		...
		if (sr(9)==1)&(max_point(n(cha))==0.000) {
			...
			add_string "<td class=\"singer\"><div class=txt>"+art(n(cha))+"</div></td>\n"
			for chb,0,m,1			
				if chb==m-1:st(4)="L":else:st(4)=""
				if (rank(n(cha),chb) >= 1 & rank(n(cha),chb) <= 3) {
					st(6)=str(rank(n(cha),chb));1-3位
